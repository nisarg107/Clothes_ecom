...
class AdminUploadItem extends StatefulWidget {
  const AdminUploadItem({super.key});

  @override
  State<AdminUploadItem> createState() => _AdminUploadItemState();
}

class _AdminUploadItemState extends State<AdminUploadItem> {
 final ImagePicker _picker = ImagePicker();
  ...
  uploadItemImage() async{
    ...
    requestImgurApi.headers['Authorization'] = "Client-ID " + "	c385672aeb35ec3";
    ...
    final jsonRes = await jsonDecode(resultFromImgurApi) as Map<String,dynamic>;
  ...
  saveIteminfotoDatabase() async{
    ...
    try {
      var response=await http.post(Uri.parse(API.uploadNewItem),
      body: {
        'item_id':'1',
      ...
      if(response.statusCode==200){
        var resBodyofUploadItem= await json.decode(json.encode(response.body));
